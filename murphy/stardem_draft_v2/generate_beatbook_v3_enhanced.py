#!/usr/bin/env python3
"""
Generate a comprehensive education beat book (VERSION 3 ENHANCED) by combining:
...
VERSION 3 ENHANCED IMPROVEMENTS:
- Uses groq/meta-llama/llama-4-maverick-17b-128e-instruct model
- Fixed MCAP data formatting (bulleted lists instead of semicolons)
- All Version 3 features maintained
"""
...
OUTPUT_FILE = "comprehensive_education_beatbook_v3_enhanced.md"
...
def format_county_data(county_name: str, county_entry: dict) -> str:
    """Convert county JSON data to readable text for prompts with bulleted MCAP data."""
    if not county_entry:
...
def format_school_data(schools: list) -> str:
    """Convert school JSON data to readable text for prompts with bulleted lists."""
    if not schools:
...
- After the narrative, include a "Quick Facts" subsection with key numbers in BULLETED FORM (use - for bullets)
...
- Include ALL subjects in MCAP data: ELA, Math, AND Science proficiency rates presented as BULLETED LISTS
...
- Under each heading, use **bulleted lists** (use - for bullets)
...
- For Key State Figures & Data: Include state-level MCAP averages and other statewide benchmarks for comparison as BULLETED LISTS. DO NOT include the governor except to note education bills signed.

- Use bulleted lists (use - for bullets) under each heading
...
def main():
    ...
    with open(OUTPUT_FILE, "w") as out:
        out.write("# Comprehensive Education Beat Book (Version 3 Enhanced)\n")
